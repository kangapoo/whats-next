...
// ---------- private access (set SITE_PASSWORD in Render to lock the site) ----------
app.use((req, res, next) => {
  if (!SITE_PASSWORD) return next(); // no password set -> site is open
  ...
  res.set("WWW-Authenticate", 'Basic realm="What Next"');
  return res.status(401).send("This app is private. Enter the password to continue.");
});
...
// RapidAPI Streaming Availability (Movie of the Night): strong per-region data + deep links.
// Free tier is a daily allowance (not paid credits). Cached so repeats don't spend the allowance.
const saCache = new Map();
async function saLookup(imdbID, country) {
  if (!RAPIDAPI_KEY || !imdbID) return null;
  const ck = imdbID + ":" + country;
  if (saCache.has(ck)) return saCache.get(ck);
  try {
    const u = new URL(`https://streaming-availability.p.rapidapi.com/shows/${imdbID}`);
    u.searchParams.set("country", country);
    const r = await fetch(u, { headers: { "X-RapidAPI-Key": RAPIDAPI_KEY, "X-RapidAPI-Host": "streaming-availability.p.rapidapi.com" } });
    if (!r.ok) return null; // quota reached or error -> caller falls back to TMDb (don't cache failures)
    ...
    for (const o of included) {
      ...
      if (!byName.has(name)) byName.set(name, { name, link: o.link || null, id: o.service?.id || null });
    }
    const services = [...byName.values()];
    const netflix = services.find((s) => /netflix/i.test(s.id || s.name));
    const result = { onNetflix: !!netflix, netflixLink: netflix?.link || null, services };
    saCache.set(ck, result);
    return result;
  } catch { return null; }
}

// Availability resolver: RapidAPI first (best data), TMDb watch-providers as automatic fallback.
async function getAvailability(media, id, imdbID, country) {
  const sa = await saLookup(imdbID, country);
  if (sa) return sa; // RapidAPI answered (authoritative for the region)
  return await tmdbProviders(media, id, country); // fallback: unlimited & free
}
...
async function enrichTitle(media, id, raw, country, reason) {
  ...
  const sa = await getAvailability(media, id, imdbID, country);
  let videos = [];
  try {
  ...
  return {
    ...
    genres: Object.keys(GENRE_IDS).filter((g) => (GENRE_IDS[g][media === "tv" ? "tv" : "movie"] || []).some((gid) => (raw.genre_ids || []).includes(gid))),
    reason: reason || "",
